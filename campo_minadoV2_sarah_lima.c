...
int noExceptions( int* plays, char **mat, int *repeat, int valid, int lin, int col, int *coordX, int *coordY){
    int end;
        ...
        while(scanf("%d,%d", coordX, coordY) == 2){
            ...
            else if((*coordX > lin)||(*coordX < 1)||(*coordY > col)||(*coordY < 1)){
...
int main() {
    ...
    while(plays >= 0){
        ...
        if(firstPlay){
            ...
            printf("%d %d\n", coordX, coordY);
            createMines(mines, lin, col, nMines, coordX -1, coordY - 1); //Subtract minus one because the coordinates chosen by the player starts with 1
